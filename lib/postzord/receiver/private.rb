class Postzord::Receiver::Private < Postzord::Receiver
  ...
  def receive!
    begin 
      if @sender && self.salmon.verified_for_key?(@sender.public_key)
        ...
        FEDERATION_LOGGER.info("event=receive status=abort recipient=#{@user.rails4bp_handle} sender=#{@salmon.author_id} reason='not_verified for key'")
        false
      end
    rescue => e
      #this sucks
  ...
  def parse_and_receive(xml)
    @object ||= Rails4Bp::Parser.from_xml(xml)
  ...
  def validate_object
    raise Rails4Bp::ContactRequiredUnlessRequest if contact_required_unless_request
    raise Rails4Bp::RelayableObjectWithoutParent if relayable_without_parent?

    assign_sender_handle_if_request

    raise Rails4Bp::AuthorXMLAuthorMismatch if author_does_not_match_xml_author?
  ...
  def xml_author
    if @object.respond_to?(:relayable?)
      ...
      xml_author = @user.owns?(@object.parent) ? @object.rails4bp_handle : @object.parent_author.rails4bp_handle
      @author = Webfinger.new(@object.rails4bp_handle).fetch if @object.author
    else
      xml_author = @object.rails4bp_handle
    end
    xml_author
  end
  ...
  def relayable_without_parent?
    if @object.respond_to?(:relayable?) && @object.parent.nil?
      FEDERATION_LOGGER.error("event=receive status=abort reason='received a comment but no corresponding post' recipient=#{@user_person.rails4bp_handle} sender=#{@sender.rails4bp_handle} payload_type=#{@object.class})")
      return true
    end
  end
  ...
  def contact_required_unless_request
    unless @object.is_a?(Request) || @user.contact_for(@sender)
      FEDERATION_LOGGER.error("event=receive status=abort reason='sender not connected to recipient' recipient=#{@user_person.rails4bp_handle} sender=#{@sender.rails4bp_handle}")
      return true 
    end
  end
  ...
  def assign_sender_handle_if_request
    ...
    if @object.is_a?(Request)
      @object.sender_handle = @sender.rails4bp_handle
    end
  end
end
