class Postzord::Receiver::Public < Postzord::Receiver
  ...
  # @return [Object]
  def save_object
    @object = Rails4Bp::Parser::from_xml(@salmon.parsed_data)
  ...
  def xml_author
    if @object.respond_to?(:relayable?)
      #this is public, so it would only be owners sending us other people comments etc
       @object.parent_author.local? ? @object.rails4bp_handle : @object.parent_rails4bp_handle
    else
      @object.rails4bp_handle
    end
  end
  ...
  def account_deletion_is_from_author
    ...
    return false if @object.rails4bp_handle != @author.rails4bp_handle
    return true
  end
