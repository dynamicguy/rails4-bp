class Postzord::Dispatcher
  ...
  # @param object [Object] The object to be sent to other Rails4Bp installations
  # @opt additional_subscribers [Array<Person>] Additional subscribers
  def initialize(user, object, opts={})
    @sender = user
    @object = object
    @xml = @object.to_rails4bp_xml
    @opts = opts
  ...
  def self.build(user, object, opts={})
    unless object.respond_to? :to_rails4bp_xml
      raise 'This object does not respond_to? to_rails4bp xml.  Try including Rails4Bp::Federated::Base into your object'
    end
  ...
  def queue_remote_delivery_job(remote_people)
    Workers::HttpMulti.perform_async(
      @sender.id,
      Base64.strict_encode64(@object.to_rails4bp_xml),
  ...
  def deliver_to_local(people)
    ...
    if @object.respond_to?(:persisted?) && !@object.is_a?(Conversation)
      ...
      people.each do |person|
        Rails.logger.info("event=push route=local sender=#{@sender.rails4bp_handle} recipient=#{person.rails4bp_handle} payload_type=#{@object.class}")
  ...
  def batch_deliver_to_local(people)
    ...
    Rails.logger.info("event=push route=local sender=#{@sender.rails4bp_handle} recipients=#{ids.join(',')} payload_type=#{@object.class}")
  end

  def deliver_to_hub
    Rails.logger.debug("event=post_to_service type=pubsub sender_handle=#{@sender.rails4bp_handle}")
  ...
  def notify_users(users)
    ...
    #temp hax
    unless object_is_related_to_rails4bp_hq?
      Workers::NotifyLocalUsers.perform_async(users.map{|u| u.id}, @object.class.to_s, @object.id, @object.author.id)
    end
  end

  def object_is_related_to_rails4bp_hq?
    (@object.author.rails4bp_handle == 'rails4bphq@joinrails4bp.com' || (@object.respond_to?(:relayable?) && @object.parent.author.rails4bp_handle == 'rails4bphq@joinrails4bp.com'))
  end
end
