module CsvGenerator
  ...
  PATH = '/tmp/'
  BACKER_CSV_LOCATION = File.join('/usr/local/app/rails4bp/', 'backer_list.csv')
  #BACKER_CSV_LOCATION = File.join('/home/ilya/workspace/rails4bp/', 'backer_list.csv')
    ...
    sql = <<SQL
          ...
          UNION
            SELECT `users`.email AS '%EMAIL%',
                    'Friend of Rails4Bp*' AS '%NAME%',
                CONCAT( 'https://joinrails4bp.com/users/invitation/accept?invitation_token=', `users`.invitation_token) AS '%INVITATION_LINK%'
                #{self.output_syntax(file)}
             FROM `users`
          ...
          UNION
            SELECT `users`.email AS '%EMAIL%',
                   IF( `profiles`.first_name IS NOT NULL AND `profiles`.first_name != "",
                                               `profiles`.first_name, 'Friend of Rails4Bp*') AS '%NAME%',
                IF(`users`.invitation_token, CONCAT( 'https://joinrails4bp.com/users/invitation/accept?invitation_token=', `users`.invitation_token) ,NULL) AS '%INVITATION_LINK%'
                #{self.output_syntax(file)}
             FROM `users`
...
SQL
  ...
  # BACKER RECENT LOGIN
  # User.where("last_sign_in_at > ?", (Time.now - 1.month).to_i).where(:email => ["maxwell@joinrails4bp.com"]).count
  #
  # "SELECT `users`.* FROM `users` WHERE `users`.`email` IN ('maxwell@joinrails4bp.com') AND (last_sign_in_at > 1312663724)"

  # NON BACKER RECENT LOGIN
  # User.where("last_sign_in_at > ?", (Time.now - 1.month).to_i).where("email NOT IN (?)", 'maxwell@joinrails4bp.com').to_sql
  # "SELECT `users`.* FROM `users` WHERE (last_sign_in_at > 1312665370) AND (email NOT IN ('maxwell@joinrails4bp.com'))"
