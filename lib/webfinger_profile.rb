    ...
    doc.css('Link').each do |l|
      ...
      case rel
        when "http://microformats.org/profile/hcard"
          @hcard = href
        when "http://joinrails4bp.com/guid"
          @guid = href
        when "http://joinrails4bp.com/seed_location"
          @seed_location = href
      end
    end

    begin
      pubkey = text_of_attribute( doc.at('Link[rel=rails4bp-public-key]'), 'href')
