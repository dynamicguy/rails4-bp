...
class Rails4BpRedis
  ...
  def initialize
    @config = Rails4BpRedis.config
    @redis = Rails4BpRedis.raw_connection(@config)
  end
  ...
  [:append, :blpop, :brpop, :brpoplpush, :decr, :decrby, :del, :exists, :expire, :expireat, :get, :getbit, :getrange, :getset,
   ...
   :zremrangebyscore, :zrevrange, :zrevrangebyscore, :zrevrank, :zrangebyscore].each do |m|
    define_method m do |*args|
      args[0] = "#{Rails4BpRedis.namespace}:#{args[0]}"
      @redis.send(m, *args)
    end
  end
  ...
  def self.new_redis_store
    ...
    redis_store.options[:namespace] = -> { Rails4BpRedis.namespace }
    redis_store
  end
