...
class Retraction
  ...
  def self.for(object)
    ...
    retraction.rails4bp_handle = object.rails4bp_handle
    retraction
  end
  ...
  def receive(user, person)
    if self.type == 'Person'
      unless self.person.guid.to_s == self.post_guid.to_s
        Rails.logger.info("event=receive status=abort reason='sender is not the person he is trying to retract' recipient=#{self.rails4bp_handle} sender=#{self.person.rails4bp_handle} payload_type=#{self.class} retraction_type=person")
        return
      end
      user.disconnected_by(self.target)
    elsif self.target.nil? || self.target.author != self.person
      Rails.logger.info("event=retraction status=abort reason='no post found authored by retractor' sender=#{person.rails4bp_handle} post_guid=#{post_guid}")
    else
      self.perform(user)
    end
    self
