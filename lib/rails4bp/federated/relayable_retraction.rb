class RelayableRetraction < SignedRetraction
  ...
  def receive(recipient, sender)
    if self.target.nil?
      ...
      Rails.logger.info("event=receive status=abort reason='object signature not valid' recipient=#{recipient.rails4bp_handle} sender=#{self.parent.author.rails4bp_handle} payload_type=#{self.class} parent_id=#{self.parent.id}")
      return
    end
    self
  end
