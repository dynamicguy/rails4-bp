...
class SignedRetraction
  ...
  def sender_handle= new_sender_handle
    @sender = Person.where(:rails4bp_handle => new_sender_handle).first
  end

  def sender_handle
    @sender.rails4bp_handle
  end
  ...
  def receive(recipient, sender)
    if self.target.nil?
      Rails.logger.info("event=retraction status=abort reason='no post found' sender=#{sender.rails4bp_handle} target_guid=#{target_guid}")
      return
    elsif self.target_author_signature_valid?
      #this is a retraction from the upstream owner
      self.perform(recipient)
    else
      Rails.logger.info("event=receive status=abort reason='object signature not valid' recipient=#{recipient.rails4bp_handle} sender=#{self.sender_handle} payload_type=#{self.class}")
      return
    end
    self
  end
