class Request
  ...
  # @return [Request]
  def self.rails4bp_initialize(opts = {})
    req = self.new
  ...
  def sender_handle= sender_handle
    self.sender = Person.where(:rails4bp_handle => sender_handle).first
  end

  # @note Used for XML marshalling
  # @return [String]
  def recipient_handle
    recipient.rails4bp_handle
  end
  def recipient_handle= recipient_handle
    self.recipient = Person.where(:rails4bp_handle => recipient_handle).first
  end
