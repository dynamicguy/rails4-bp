  ...
  module Relayable
    ...
    def self.included(model)
      model.class_eval do
        ...
        delegate :author, :rails4bp_handle, to: :parent, prefix: true
    ...
    def receive(user, person=nil)
      ...
      unless comment_or_like.parent_author == user.person || comment_or_like.verify_parent_author_signature
        Rails.logger.info("event=receive status=abort reason='object signature not valid' recipient=#{user.rails4bp_handle} sender=#{self.parent.author.rails4bp_handle} payload_type=#{self.class} parent_id=#{self.parent.id}")
        return
      end
