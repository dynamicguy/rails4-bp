@Rails4Bp.module "CompaniesApp.List", (List, App, Backbone, Marionette, $, _) ->
  class List.Controller extends App.Controllers.Base
    ...
    initialize: ->
      companies = App.request "company:entities"
      App.execute "when:fetched", companies, =>
        @layout = @getLayoutView companies
        @listenTo @layout, "close", @close
        @listenTo @layout, "show", =>
          @breadcrumbRegion()
          @sidebarRegion()
          @panelRegion()
          @listRegion companies
        @show @layout
    ...
    panelRegion: ->
      ...
      @listenTo panelView, "new:company:button:clicked", =>
        @newRegion()
      @layout.panelRegion.show panelView

    newRegion: ->
      App.execute "new:company", @layout.newRegion

    listRegion: (companies) ->
      listView = @getListView companies
      @listenTo listView, "childview:company:clicked", (child, args) ->
        App.vent.trigger "company:clicked", child.model
      @layout.listRegion.show listView

      ActionsCell = Backgrid.Cell.extend(
        className: 'actions'
        template: _.template("<a href='#companies/<%= id %>' class='btn btn-default btn-xs'><span class='glyphicon glyphicon-eye-open'></span></a> <a href='#companies/<%= id %>/edit' class='btn company-edit btn-default btn-xs'><span class='glyphicon glyphicon-pencil'></span> </a> <button data-id='<%= id %>' class='company-delete btn btn-danger btn-xs' data-type='confirm'><span class='glyphicon glyphicon-trash'></span></button>")
        events:
