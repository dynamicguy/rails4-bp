@Rails4Bp.module "Entities", (Entities, App, Backbone, Marionette, $, _) ->
	class Entities.Model extends Backbone.Model
		destroy: (options = {}) ->
			...
			@set _destroy: true
			super options
			Rails4Bp.Notify.show("success","item with \"name\" \"#{@.get('name')}\" has been destroyed successfully.","bottomRight")
		...
		saveSuccess: (isNew, collection) =>
			if isNew ## model is being created
				...
				@trigger "created", @
				Rails4Bp.Notify.show("success","item with \"name\" \"#{@.get('name')}\" has been created successfully.","bottomRight")
				...
				@trigger "updated", @
				Rails4Bp.Notify.show("success","item with \"name\" \"#{@.get('name')}\" has been saved successfully.","bottomRight")
		...
		saveError: (model, xhr, options) =>
			...
			Rails4Bp.Notify.show("error","Something went wrong while saving row with \"name\" \"#{@.get('name')}\". Please try again.","bottomRight")
