module MarkdownifyHelper
  ...
  def markdownify(target, render_options={})
    ...
    renderer = Rails4Bp::Markdownify::HTML.new(render_options)
    markdown = Redcarpet::Markdown.new(renderer, markdown_options)

    message = markdown.render(message).html_safe

    if target.respond_to?(:mentioned_people)
      message = Rails4Bp::Mentionable.format(message, target.mentioned_people)
    end

    message = Rails4Bp::Taggable.format_tags(message, :no_escape => true)
